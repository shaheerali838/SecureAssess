...
export function ExaminerLiveInterview({ onNavigate }) {
  ...
  const handleHostNextCandidate = () => {
    // Reset rubric ratings for fresh candidate viva evaluation
    setQuestions((prev) =>
      prev.map((q) => ({
        ...q,
        rating: 0,
    ...
    // Notify room / signaling that examiner is ready to viva next student in queue
    if (socketRef.current) {
  ...
  // =========================================================================
  // VIEW: EXAMINER INTERVIEW CONCLUDED & EVALUATION SUMMARY
  // =========================================================================
  if (sessionEnded) {
          ...
          <div className="p-8 rounded-3xl bg-accent-900 border border-accent-800 shadow-2xl space-y-6 w-full relative overflow-hidden">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-accent-300 leading-relaxed">
                The evaluation scores, rubrics, and notes for{' '}
                <span className="text-white font-semibold">{examineeName}</span> have been finalized and recorded in the database.
              </p>
            </div>
            ...
            {/* Rubric Breakdown List with Interactive Scoring */}
            <div className="p-4 rounded-2xl bg-accent-950/60 border border-accent-800 text-left space-y-3">
              <div className="flex items-center justify-between border-b border-accent-800/80 pb-2">
                ...
                <span className="text-[11px] text-accent-400">
                  Click stars below to score or adjust question grades
                </span>
              </div>
      ...
      <header className="bg-accent-900 border-b border-accent-800 px-5 h-14 flex items-center justify-between shrink-0 z-20">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs px-2.5 py-1 rounded bg-accent-800 hover:bg-accent-700 text-accent-300 transition-colors"
