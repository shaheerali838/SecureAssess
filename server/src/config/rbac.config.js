...
import { PLATFORM_ROLES, ORGANIZATION_ROLES } from "../constants/roles.js";
...
/**
 * Express Authorization Middleware enforcing Scope & Tenant Isolation
 */
export const requirePermission = (permission) => {
  return async (req, res, next) => {
    ...
    if (platformRole === ROLES.PLATFORM_ADMIN) {
      const adminPerms = ROLE_PERMISSIONS[ROLES.PLATFORM_ADMIN] || [];
      ...
      return next(
        new ApiError(
          403,
          "Forbidden. Platform administrators do not have direct access to customer tenant operational data."
        )
      );
    }
    ...
    if (!targetOrgId && !permission.startsWith("candidate.")) {
      return next(new ApiError(400, "Organization ID header (x-organization-id) is required for tenant operations"));
    }
      ...
      if (targetOrgId) {
        ...
        if (!membership) {
          AuditLogService.createSecurityAuditLog({
            ...
            description: `User attempted cross-tenant access without active membership`,
          ...
          return next(
            new ApiError(403, "Forbidden. You do not have active membership in this organization.")
          );
        }

        userRole = membership.roleId?.name || (typeof membership.roleId === "string" ? membership.roleId : "");
      } else {
        userRole = req.user.role || req.user.roleName || "CANDIDATE";
      }

      const normalized = (userRole || "").toUpperCase();

      const assignedPerms = ROLE_PERMISSIONS[normalized] || [];
      if (!assignedPerms.includes(permission)) {
        AuditLogService.createSecurityAuditLog({
          organizationId: targetOrgId,
          actorId: req.user.id || req.user._id,
          ...
          resource: AUDIT_RESOURCES.ORGANIZATION,
          resourceId: targetOrgId,
          description: `User lacks required tenant permission: '${permission}'`,
          metadata: { targetOrgId, requiredPerm: permission, role: normalized, path: req.originalUrl, method: req.method },
          ipAddress: req.ip || req.connection?.remoteAddress,
          userAgent: req.headers["user-agent"],
          requestId: req.requestId,
          status: AUDIT_STATUSES.DENIED,
        ...
        return next(
          new ApiError(403, `Forbidden. Missing required permission: '${permission}'`)
        );
      }
